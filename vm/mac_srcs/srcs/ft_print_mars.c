...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/06 17:01:28 by hmassonn          #+#    #+#             */
/*   Updated: 2017/03/06 17:01:31 by hmassonn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_pid_color(t_mars **mars, int pid, t_cpu *cpu, int n)
{
	...
	while (cpu)
	{
		if (cpu->pc == n && cpu->live >= 0 && (fill = true))
			break ;
		cpu = cpu->next;
	}
	ft_addbuf(mars, "\033[0m");
...
void	ft_print_mars(t_mars **mars, t_cpu *cpu)
{
	...
	while (++i < MEM_SIZE)
	{
		if (incode && !(incode = (*mars)->arena_color[i]))
			...
			ft_print_pid_color(mars, (*mars)->arena_color[i], cpu, i);
