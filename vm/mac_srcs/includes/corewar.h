...
/*   Updated: 2017/03/07 11:55:43 by clanier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# include <ncurses.h>						// for mvaddstr
# include "inc/fmod.h"						// for FMOD
...
typedef struct				s_mars
{
	...
	unsigned int			total_size;		// a tab with every players uid in game
	char				last_color;		// the id of the last color in ncurses mode
	t_mlx				*mlx;			// pointer to the mlx struct for the mlx informations
	t_sound				*sound;			// pointer to the sound struct for the fmod informations
}					t_mars;

/*
 * 00000001 for -p <= basic display with write(3)
 * 00000010 for -g <= graph version with mlx
 * 00000100 for -w <= graph version with ncurses
